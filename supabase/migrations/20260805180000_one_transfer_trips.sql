-- ============================================================================
-- Single-transfer trip search, the natural extension of
-- `search_direct_trips` (see `20260805090000_transit_query_functions.sql`).
--
-- Still not a *general* multi-transfer router - it only ever considers
-- exactly one line change, at a station reachable by a direct trip from the
-- origin AND a direct trip to the destination. That covers the overwhelming
-- majority of real KRL Jabodetabek journeys (this network is a small number
-- of radial lines meeting at a handful of interchanges - Manggarai, Duri,
-- Tanah Abang, Kampung Bandan - not a dense grid needing 2+ transfers), but
-- a station pair that genuinely needs two or more changes still returns
-- nothing here, same honesty posture as `search_direct_trips`.
-- ============================================================================
...
comment on function public.search_one_transfer_trips is
  'Trip candidates between two stations requiring exactly one line change, found by joining a direct-from-origin leg to a direct-to-destination leg at a shared transfer station, respecting p_transfer_buffer_seconds as the minimum dwell. Not a general router - see the file header comment.';
